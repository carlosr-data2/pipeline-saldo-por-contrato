...
A demonstração local não pode depender de rede, credencial ou disponibilidade
de nuvem — a regra de ouro de qualquer demo. Ao mesmo tempo, cabe perguntar
explicitamente pelo equivalente local do Glue Data Catalog. Isso coloca a
...
- **Catálogo por configuração**: uma única função (`criar_spark` em
  ...
  **HadoopCatalog** — os metadados das tabelas vivem no filesystem, cumprindo
  ...
  com warehouse em S3. Nenhuma linha de job muda entre as trilhas.
...
- A demo é **determinística e offline** — não tem
