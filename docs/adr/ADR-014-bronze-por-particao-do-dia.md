...
Na primeira versão, o Bronze lia o CSV único de exemplo inteiro a cada execução e
sobrescrevia todas as partições presentes nele. Funciona para os três dias do
caso, mas não sustenta a narrativa do fechamento diário: a Step Function resolvia
o `dt` e o passava ao Silver e ao Gold, e o Bronze não recebia nada — não sabia
...
origem como **Parquet particionado por `dt_processamento`**; o CSV único é o
formato do dataset de exemplo, não o da produção.
...
4. **Partição vazia na origem falha alto, no Bronze.** Antes, um dia sem lote só
...
5. **O commit vira log.** O evento `bronze_commit` publica o resumo do snapshot
   Iceberg recém-criado (`changed-partition-count`, `added-records`,
   `added-data-files`), lido dos metadados da própria tabela: a prova de que uma
...
- O retry do Bronze na Step Function (2 tentativas, pensado para I/O
  ...
  o mesmo trade-off registrado para o Silver e o Gold no ADR-002. A saída, se
  isso incomodar, é a mesma proposta lá: decidir na state machine em vez de
  dentro do job.
- A contagem antes da escrita custa uma leitura extra da partição do dia (não do
  histórico). Aceito: é o preço de não deixar `overwritePartitions` virar no-op
  em silêncio com um DataFrame vazio.
