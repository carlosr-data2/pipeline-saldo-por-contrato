...
Esta ADR decide o que o pipeline deve fazer com um problema real que o
contrato não pediu para tratar.

## Decisão

Três respostas, uma por camada de responsabilidade:

1. **A regra do contrato continua exata**: existência no domínio, e quarentena
   para quem falha. Nem mais, nem menos do que o contrato define.
2. **A incoerência vira métrica**: o relatório de qualidade
...
3. **A pergunta volta ao data owner com evidência**: "83% dos lançamentos têm
   ...
   virar regra?". Ela está registrada como uma das dúvidas formais de contrato.

## Alternativas rejeitadas

**1. Promover a coerência a regra de bloqueio.** O efeito prático: 83% do
dataset na quarentena, e o fechamento nunca aconteceria. Além disso, seria o
pipeline inventando uma regra que o contrato não pediu. Regra de qualidade
nasce de acordo com o owner, não de iniciativa unilateral do engenheiro.

**2. Reclassificar silenciosamente** (sobrescrever o `cod_cosif` pelo código
associado ao tipo de contrato). É a opção mais perigosa: um pipeline
"corrigindo" classificação contábil por conta própria está adulterando dado
