...
seria um `dropDuplicates`, mas a análise exploratória do dataset mudou o
problema por completo. Dois achados:
...
1. **Nenhuma duplicata é uma cópia idêntica.** Dos 2.736 ids duplicados, 100%
   ...
   diferentes sob o mesmo id. Não são reentregas do mesmo registro (que
   poderiam ser colapsadas sem perda), e sim colisões: duas transações
...
em silêncio, o que é inaceitável em dado regulatório, onde toda exclusão
precisa de motivo e rastro. Escolher qual registro sobrevive virou, portanto,
...
2. **Linha inválida não participa da disputa.** Um registro que já violou outra
   ...
   ele não pode "vencer" a janela e, principalmente, não pode condenar uma
...
4. **Reenvio corrigido é aceito.** Esta é a consequência deliberada com mais
   peso operacional: como a comparação histórica olha só o *publicado*, o
   reenvio corrigido de um registro quarentenado passa. O fluxo natural de
   correção (upstream conserta o registro rejeitado e reenvia com o mesmo id)
   funciona; a quarentena existe para permitir correção, não para bloquear um
   id definitivamente.

Como em todo o pipeline, perdedores nunca são descartados: vão para a
...
**2. Quarentenar o grupo inteiro em caso de colisão.** Parece mais
conservador, mas é pior na prática: junto com o registro suspeito, joga fora o
...
regulatório, toda remoção precisa de um "removi X porque Y".
...
- **Detalhe de implementação** que a política exige: a ordenação da janela usa
  `valida DESC, dt_lancamento ASC, hash ASC`. O `valida DESC` garante a regra 2
  (inválidas ordenam depois de todas as válidas, então nunca ocupam a posição
  de vencedora).
