...
## Contexto: onde uma fila ajuda — e onde só adiciona um lugar para dado sumir
...
próprio orquestrador. A pergunta desta decisão, então, foi honesta: *existe
algum papel real para uma fila neste desenho?* A resposta: um só, e pequeno.
...
- **A única fila do desenho**: uma DLQ (SQS) no alvo do EventBridge Scheduler.
  ...
  uma linha de Terraform e zero por mês; é o seguro contra a perda silenciosa
  do disparo.
...
mínimo, mas fora do escopo da prova de conceito. Trade-off declarado em
`docs/arquitetura.md`, não escondido.
