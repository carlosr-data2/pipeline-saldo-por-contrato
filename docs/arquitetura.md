...
do dia na origem Parquet e sobrescreve só ela (ADR-014). Cada estágio grava
tabelas Iceberg no S3, catalogadas
...
reexecuta *do estágio que falhou*, sem repetir o que já passou. Essa é a razão de
...
Porque ela é a chave do lote, e o lote é a unidade de tudo neste pipeline:
...
- **Skew (contas concentradas)**: três camadas: o estágio mais pesado (janela de
  ...
  conta, a alternativa documentada é salting (agregação parcial por conta+sal,
  ...
  gargalo.
- **Cache/persist**: a regra é cachear o que alimenta múltiplos consumidores no
