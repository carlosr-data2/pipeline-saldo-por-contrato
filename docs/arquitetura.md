...
**Trilha local:** os mesmos 3 jobs em Docker (Spark 3.5.4 +
...
Por que `dt_processamento` como partição: é a chave do LOTE — unidade de
...
A especificação traz "< 1 hora" e uma janela 22h→02h (4h) — lidas como camadas de um
...
da Step Function + redrive); as 4h finais são contingência operacional.

## Escala de produção: ~300M transações/dia, ~80M contas
...
**Onde cada otimização pedida atua:**
