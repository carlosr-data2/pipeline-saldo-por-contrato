...
perdido), quem grita são os **alarmes-sentinela**, desenhados para transformar
...
- **Nada é descartado em silêncio.** O bronze guarda *tudo* que chegou; o que
  ...
  `bronze = silver + quarentena` (no dataset de exemplo: 66.666 = 61.289 + 5.377
...
A especificação traz um SLA de processamento "< 1 hora" e uma janela de 22h→02h
...
### O insight central: incremental, nunca full scan
...
2. **Anti-join de unicidade limitado à janela de lookback** (7 dias, configurável)
   — verificar `id_transacao` contra 5 anos de histórico custaria um join de
   300M contra 550 bilhões; contra 7 dias, custa 300M contra ~2 bilhões (ADR-006);
...
- **Cache/persist** — a regra: cachear o que alimenta **múltiplos consumidores no
  ...
  disco) — em produção, é isso que puxa o G.2X — e a disciplina do `unpersist()`.
...
## Observabilidade: projetada para o silêncio, não só para o erro

A pergunta que guiou este bloco: *como eu descubro um problema às 3h da manhã —
inclusive o problema de nada ter rodado?*
...
3. **Alarmes por ausência** (o diferencial): "nenhuma execução iniciada em 24h" e
   ...
   *falta* da métrica também dispara, então até um agendador desligado gera
   ...
   sumir. Limitação declarada: a granularidade é diária; a checagem fina "22:15
