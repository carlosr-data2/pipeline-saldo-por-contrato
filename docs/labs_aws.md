...
A primeira versão da trilha AWS mostrou a arquitetura funcionando
([`docs/evidencias.md`](evidencias.md)). Estes laboratórios usam a mesma conta
para provocar cenários e observar o que a plataforma faz no S3, no Data Catalog,
nos logs e no histórico da Step Function. Cada lab diz o que rodar, o que olhar e
o que anotar.
...
- Infraestrutura provisionada (seção 1 do runbook) e inscrição do SNS confirmada.
...
- Cron desligado durante os labs: `terraform apply -var agendamento_ativo=false`.
  ...
  dia sem execução.
- Estimativa de custo (fórmula da seção 2 do runbook): Lab 1 ≈ 6 execuções × US$ 0,14 <
...
3. Execute os três dias em ordem (seção 3 do runbook). No CloudWatch Logs Insights, no log
   group do job Bronze:
   ```
