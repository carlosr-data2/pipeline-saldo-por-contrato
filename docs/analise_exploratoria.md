...
1. **Duplicatas não são reentregas, e a dedup virou ADR.**
Achado: 2.736 grupos de id_transacao duplicado e nenhum grupo com conteúdo idêntico. São payloads diferentes disputando o mesmo id. Consequência: dropDuplicates() (a solução de uma linha) escolheria o vencedor por sorteio, o que é inaceitável em contabilidade. Daí o ADR-006: vencedor determinístico (dt_lancamento + hash como desempate) e perdedores para a quarentena com motivo. Sem a EDA, dropDuplicates pareceria suficiente.

2. **71% das duplicatas cruzam partições, e daí veio o lookback.**
Achado: 1.940 dos 2.736 grupos têm as cópias em dias diferentes. Consequência: dedup só dentro do lote do dia deixa passar a maioria, então o incremental diário precisa checar contra o que já foi publicado. Daí o anti-join contra a silver numa janela de 7 dias, e a renegociação do "único globalmente" (inviável a 300M/dia × 5 anos) para "unicidade em janela". Esse número (71%) definiu um componente inteiro do silver.

3. **Os volumes de violação calibraram o gate.**
Achados: valor≤0 (3.989), COSIF fora do domínio (3.309), data futura (3.257), conta nula (2.411), total de ~8,1% de linhas problemáticas. Duas consequências: (a) a quarentena tinha que guardar tudo com motivo, porque 8% de descarte silencioso é um buraco contábil; (b) o limiar do gate ficou em 10%: o dataset real passa (8,1% < 10%), e o bloqueio se prova por teste com limiar artificial. O 10% foi calibrado no dado.

4. **A incoerência de 83% virou dado, não bloqueio.**
Achado: 83,3% dos registros válidos têm cod_cosif incoerente com o tipo_contrato. Consequência: se isso fosse regra de bloqueio, o pipeline rejeitaria o dataset quase inteiro. Então não é violação do contrato (que só exige existir no domínio); é uma pergunta ao data owner. Daí o ADR-008: a flag_coerente na gold classificacao_cosif expõe a incoerência como dado consultável. A EDA evitou inventar uma regra que o contrato não pediu e que teria travado o pipeline.

5. **Sinal não definido + 5 tipos de lançamento levaram à convenção do ADR-007.**
Achado: tipo_lancamento tem 5 valores e o contrato não diz quem soma e quem subtrai; sem isso, não existe saldo. Consequência: convenção assumida e documentada (CREDITO/JUROS = +; DEBITO/TARIFA/IOF = −; estorno inverte), com oráculo independente validando. A EDA mostrou que o requisito central estava incompleto antes de escrever uma linha do gold.

6. **Sem saldo de abertura, o snapshot parte do vazio (ADR-005).**
Achado: os 3 dias são só movimento; não há posição inicial. Consequência: o primeiro dia parte de snapshot vazio, e é por isso que os saldos saem negativos no dataset sintético, o que aparece em qualquer consulta à gold.
...
| `dt_lancamento` posterior ao processamento | 3.257 | quarentena pela regra de consistência de datas; também mostra que a data de negócio chega fora de ordem, o que decide a partição por `dt_processamento` |
...
**Payload divergente.** O id é a etiqueta da linha; o payload é todo o
resto: valor, conta, datas, agência, tipo. Payload divergente significa mesma
etiqueta com conteúdo diferente, ou seja, outra transação sob o mesmo id.
...
aparecer, que depende da ordem física de leitura, e duas execuções
...
O referencial cosif_dominio.csv tem 8 códigos válidos. Se o registro traz 9.9.9.99.9, que não está na lista, é violação e vai para a quarentena. Foram 3.309.

Pergunta 2 (não é regra do contrato): "esse código existe, mas é do tipo certo?"
No referencial, cada código tem um tipo_contrato_associado: 1.3.1.00.0 é "Títulos - CDB", associado ao tipo CDB; 1.2.1.00.0 é "Poupança", associado ao tipo POUP. A incoerência é um contrato de poupança carregando o código contábil de CDB. O código é real, existe no domínio e passa na regra 1, mas a combinação é estranha.

Uma analogia: é um CEP válido de outra cidade. O CEP existe nos Correios (passa na validação "CEP existe"), mas é de São Paulo num endereço de Curitiba. A carta não é devolvida; a esquisitice é anotada e a pergunta vai para quem mantém o cadastro.
...
E 83,3% é exatamente 5/6. Há 6 tipos de contrato; se o gerador do dataset sorteou o código independente do tipo, a chance de acertar o tipo por acaso é ~1/6, e é precisamente o que se observa (16,7% coerente).

Ou seja, nesse dado sintético código e tipo foram sorteados separadamente. Isso reforça a decisão: o número não diz que 83% do banco está errado, diz que a relação código×tipo desse dataset não carrega informação, e quem decide o que fazer é o dono do dado.
...
Se bloqueasse, 83% do banco iria para a quarentena e o fechamento não sairia. Uma regra de qualidade que reprova quase tudo indica que a premissa está errada.
...
Código inexistente é violação e bloqueia; código de outro tipo é anomalia e vira flag. A primeira é regra do contrato, a segunda é pergunta ao dono do dado.
...
- **Regra de qualidade nasce do contrato.** O achado mais
  grave (83% de incoerência COSIF) não virou bloqueio porque o
